...
`maid-outcome-enrich` ships through both Claude and Codex init payloads. It is
the AI-calling counterpart to the deterministic `maid enrich` command: the
...
blocking-versus-advisory classification, verdict-neutral full-delta reviewer
packets, issue-driven continuation while actionable findings remain, batched
fixes, and task-scoped inner-loop verification. Reviewer prompts never carry
prior findings, fix lineage, round state, or approval-oriented framing;
convergence remains the coordinator's post-review decision. Changes to init-distributed
