...
Python is always supported. TypeScript/JavaScript and Svelte require optional dependencies (`pip install maid-runner[typescript]`). React `.tsx`/`.jsx` files are supported through the TypeScript parser: function components, typed const components, custom hooks, provider functions, props interfaces/types, common `memo`/`forwardRef` wrapper exports, Testing Library component references, lazy `import()` calls, path aliases, and local CSS module imports are covered. Snapshot generation records existing relative React style and static asset imports as read files, but CSS/assets and React runtime, DOM, React Native, Next.js, Remix, Vite, or bundler semantics are not parsed as MAID artifacts. Angular `.ts` files are also TypeScript-backed: decorated classes, fields, methods, standalone imports, lazy route `import()` calls, and literal `templateUrl`, `styleUrl`, and `styleUrls` snapshot companions are covered without Angular compiler semantic analysis. More languages coming.
