"""Focused knockout stays available when only sibling fixtures are unproven.

Contract: manifests/drafts/121-32-focused-knockout-despite-sibling-unproven-lifecycles.manifest.yaml
"""
...
SIBLING_UNPROVEN = "fixture:tests/other.py:autouse:function:tests/other.py::test_other"


def test_bundle_sibling_unproven_lifecycle_still_uses_focused_knockout(
    tmp_path: Path,
) -> None:
    ...
    evidence = replace(
        ...
        completeness=RuntimeEvidenceCompleteness(
            ...
            unproven_fixture_lifecycles=(SIBLING_UNPROVEN,),
        ),
        commands=(
            replace(
                evidence.commands[0],
...
def test_command_unproven_lifecycle_still_forces_exact_knockout_fallback(
    tmp_path: Path,
) -> None:
    from maid_runner.core.knockout import run_knockout_batch
