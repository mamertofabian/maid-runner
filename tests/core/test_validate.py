...
def test_validate_all_reuses_artifact_collection_for_overlapping_typescript_manifests(
    tmp_path,
):
    class CountingTypeScriptValidator(BaseValidator):
        ...
        def collect_implementation_artifacts(self, source, file_path):
            ...
            return CollectionResult(
                [FoundArtifact(kind=ArtifactKind.FUNCTION, name="shared")],
                "typescript",
                str(file_path),
            )
    ...
    (tmp_path / "src" / "shared.ts").write_text("export function shared() {}")
    (tmp_path / "tests" / "shared.test.ts").write_text("shared()")
    for slug in ("first", "second"):
        _write_manifest(
            tmp_path / "manifests",
            f"{slug}.manifest.yaml",
            """schema: "2"
...
""",
    ...
    registry.register(CountingTypeScriptValidator)
    ...
    assert result.success is True
    assert CountingTypeScriptValidator.implementation_calls == 1
    assert CountingTypeScriptValidator.behavioral_calls == 1
