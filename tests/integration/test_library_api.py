...
class TestPublicAPIImports:
    ...
    def test_release_metadata_is_2_21_0(self):
        ...
        assert re.search(r'^version = "2\.21\.0"$', pyproject.read_text(), re.M)
        assert re.search(
            r'\[\[package\]\]\nname = "maid-runner"\nversion = "2\.21\.0"',
            lockfile.read_text(),
        )
        changelog_text = changelog.read_text()
        assert "## [2.21.0] - 2026-07-11" in changelog_text
        assert (
            "[2.21.0]: https://github.com/mamertofabian/maid-runner/compare/v2.20.0...v2.21.0"
            in changelog_text
        )
        assert "**Current Version:** 2.21.0" in roadmap.read_text()
        assert "The local CLI reports `maid 2.21.0`." in roadmap.read_text()
