...
class TestPublicAPIImports:
    ...
    def test_release_metadata_is_2_20_0(self):
        from maid_runner import __version__
        ...
        assert re.search(r'^version = "2\.20\.0"$', pyproject.read_text(), re.M)
        assert re.search(
            r'\[\[package\]\]\nname = "maid-runner"\nversion = "2\.20\.0"',
            lockfile.read_text(),
        )
        changelog_text = changelog.read_text()
        assert "## [2.20.0] - 2026-07-05" in changelog_text
        assert (
            "[2.20.0]: https://github.com/mamertofabian/maid-runner/compare/v2.19.1...v2.20.0"
            in changelog_text
        )
        assert "**Current Version:** 2.20.0" in roadmap.read_text()
        assert "The local CLI reports `maid 2.20.0`." in roadmap.read_text()
