...
class TestPublicAPIImports:
    ...
    def test_release_metadata_is_2_19_1(self):
        from maid_runner import __version__
        ...
        assert re.search(r'^version = "2\.19\.1"$', pyproject.read_text(), re.M)
        assert re.search(
            r'\[\[package\]\]\nname = "maid-runner"\nversion = "2\.19\.1"',
            lockfile.read_text(),
        )
        assert "## [2.19.1] - 2026-07-01" in changelog.read_text()
        assert "**Current Version:** 2.19.1" in roadmap.read_text()
        assert "The local CLI reports `maid 2.19.1`." in roadmap.read_text()
