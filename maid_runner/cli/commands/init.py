...
def _render_draft_outcome_guidance() -> str:
    return (
        ...
        "changes while implementation is uncommitted, use "
        '`uv run maid plan revise <manifest> --reason "<text>" '
        "--stash-implementation` so MAID temporarily hides declared "
        "implementation changes while it captures fresh red evidence. "
        "This cannot hide committed implementation. If revised tests fail "
        "for missing behavior in committed code, plain `maid plan revise` "
        "can capture fresh red evidence before the fix. If they already pass "
        "and valid evidence cannot be preserved, report the evidence blocker "
        "and the pre-implementation baseline needed for recovery; do not "
        "reset shared history or bypass evidence requirements. See "
        "'Revision Evidence After Implementation' in "
        "`docs/draft-manifest-workflow.md`. "
        "While a draft or promoted contract belongs to the current unmerged "
        "task, revise that same contract in place; use `maid plan revise` "
        "when it is locked. A contract is durable when accepted into shared "
        "project history, including integration or release branches regardless "
        "of name; a local task commit alone is not acceptance. Check repository "
        "policy and history, and clarify uncertain acceptance before rewriting. "
        "Use new manifests for genuinely separate work or evolution of durable "
        "contracts through chain merging or `maid-evolver` supersession.\n\n"
        "Declare each touched source file in `files.create`, `files.edit`, "
        "`files.scope`, or `files.delete`; `files.read` is context, not "
        "writable scope. When public behavior changes in a touched file, add "
        "focused behavioral or characterization tests and declare any "
        "affected public artifacts in `files.create` or `files.edit`, even if "
        "the file was previously listed only in `files.scope`. Use "
        "`files.scope` for narrow no-artifact wiring covered by behavioral "
        "tests instead of forcing "
        "a whole-file public artifact snapshot.\n\n"
