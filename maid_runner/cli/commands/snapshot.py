"""CLI handler for 'maid snapshot' and 'maid snapshot-system' commands."""

from __future__ import annotations

import argparse

from maid_runner.cli.commands._format import print_error


def cmd_snapshot(args: argparse.Namespace) -> int:
    try:
        from maid_runner.core.snapshot import generate_snapshot, save_snapshot
    except ImportError:
        print_error("Snapshot module not available yet.")
        return 2

    try:
        manifest = generate_snapshot(args.file_path)
        if args.dry_run:
            from maid_runner.cli.commands._format import format_manifest

            print(format_manifest(manifest, json_mode=args.json))
            return 0
        path = save_snapshot(
            manifest, output_dir=args.output_dir, output=args.output, force=args.force
        )
        print(f"Snapshot saved to {path}")
        return 0
    except Exception as e:
        print_error(str(e))
        return 2


def cmd_snapshot_system(args: argparse.Namespace) -> int:
    try:
        from maid_runner.core.snapshot import generate_system_snapshot
    except ImportError:
        print_error("Snapshot module not available yet.")
        return 2

    try:
        result = generate_system_snapshot(
            manifest_dir=args.manifest_dir, output=args.output
        )
        if not args.quiet:
            print(result)
        return 0
    except Exception as e:
        print_error(str(e))
        return 2
