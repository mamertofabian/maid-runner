...
8. Before handoff, run `uv run maid assess --since <baseline>`, then run the exact verify command it prints. Assessment scales between `handoff` (`--summary --require-plan-lock --require-red-evidence`) and `deep`; its deep command retains those requirements. If assessment is unavailable or fails, use `uv run maid verify --profile handoff --since <baseline>` as the safety floor. Treat plan-lock or red-evidence failures as workflow blockers rather than recreating evidence after implementation.
