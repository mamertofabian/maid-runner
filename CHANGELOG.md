...
### Fixed
- **TypeScript type formatting** — Compare valid TypeScript type aliases structurally so formatter-expanded objects, functions, intersections, unions, comments, semicolon placement, and ordinary string quote styles do not produce false `E302` mismatches. Semantic type changes still fail, while malformed, unsupported, or resource-heavy fragments retain the bounded fail-closed comparison behavior.
...
### Fixed
- **Claude MAID skill repo install** — `maid init --tool claude` installs the current MAID-only Claude skills, agents, commands, and marked `CLAUDE.md` guidance for shared team repositories.
- **Python 3.10 CI compatibility** — Test coverage for package data uses a `tomli` fallback when `tomllib` is unavailable.

## [2.7.0] - 2026-05-07

### Added
- **Compiler-backed TypeScript identity resolution** — Added a TypeScript compiler resolver bridge for source-accurate symbol identity across tsconfig paths, workspace package boundaries, package imports, barrels, and re-exports.
- **TypeScript artifact edge-case extraction** — Added support for computed property artifacts, type alias targets, and generic type parameter storage so manifest declarations can model richer TypeScript surfaces.

### Changed
- **TypeScript resolver packaging** — Bundles the compiler resolver runtime with the Python package and adds TypeScript as a private development dependency for identity-resolution tests.

### Fixed
- **TypeScript resolver cache stability** — Fixed compiler resolver caching so repeated validations reuse resolution state without leaking incorrect identities between files or projects.

## [2.6.0] - 2026-05-06

### Added
- **TypeScript identity edge-case coverage** — Added characterization and implementation support for package imports, namespace re-exports, recursive barrels, workspace package boundaries, `tsconfig` path aliases, and ESM/CJS barrel identities so artifact matching remains source-accurate across common module topologies.

### Changed
- **TypeScript extraction depth** — Expanded implementation artifact extraction to better handle overload signatures, decorated declarations, anonymous default exports, computed method names, and generic base-type formatting while preserving the existing validator API.

### Fixed
- **TypeScript import/reference scanning accuracy** — Replaced brittle regex-driven required-import and re-export identity paths with parser-backed logic to reduce false matches and align behavioral validation with real symbol resolution.
...
### Fixed
- Consistent superseded manifest handling across validation modes
  - Both JSON and non-JSON output modes now properly skip validation for superseded manifests
  - Previously, non-JSON mode would attempt validation and report errors for inactive manifests
  - Added early supersession check in `run_validation()` before schema validation
  - Returns success with informational message when manifest is superseded
  - Ensures consistent behavior for LSP integration and CLI usage

## [0.9.3] - 2026-01-12

### Added
- Helpful hint in validation errors when supersede is needed
  - When validation fails with "Unexpected public" errors due to newer manifests modifying the same file
  - Hint explains the fix: add `supersedes` field and redeclare all artifacts
  - Improves user experience by guiding users through MAID supersession workflow

### Changed
- Moved Claude Code hooks to maid-runner plugin (removed from project `.claude` directory)
- Moved agent documentation to dedicated `AGENTS.md` file
...
[2.5.2]: https://github.com/mamertofabian/maid-runner/compare/v2.5.1...v2.5.2
